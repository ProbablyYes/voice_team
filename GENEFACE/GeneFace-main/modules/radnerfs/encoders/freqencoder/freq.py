...
class FreqEncoder(nn.Module):
    ...
    def forward(self, inputs, **kwargs):
        ...
        if _backend is None or not inputs.is_cuda:
            # Pure PyTorch implementation
            B = inputs.shape[:-1]
            C = inputs.shape[-1]
            
            flat_inputs = inputs.reshape(-1, C)
            
            out = [flat_inputs]
            for i in range(self.degree):
                freq = 2.0 ** i
                out.append(torch.sin(freq * flat_inputs))
                out.append(torch.cos(freq * flat_inputs))
                
            outputs = torch.cat(out, dim=-1)
            outputs = outputs.reshape(list(B) + [self.output_dim])
            return outputs
