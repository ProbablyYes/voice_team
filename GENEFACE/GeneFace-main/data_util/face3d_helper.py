...
class Face3DHelper:
    ...
    def load_3dmm(self):
        try:
            ...
            # Fallback to 01_MorphableModel.mat if BFM_model_front.mat is missing
            # Note: This is a simplified fallback, real conversion might be needed
            print(f"Warning: BFM_model_front.mat not found in {self.bfm_dir}. Checking for alternatives...")
            if os.path.exists(os.path.join(self.bfm_dir, "01_MorphableModel.mat")):
                 print("Found 01_MorphableModel.mat. Please ensure you have run the conversion script to generate BFM_model_front.mat.")
                 # For now, we re-raise to stop execution as the format is likely different
                 raise
            else:
                 raise
